...
/**
 * TestSuite for the hext.flow.async.Dispatcher class.
 *
 * TODO: async specific tests
 * TODO: mock ThreadExecutor
 */
class TestDispatcher extends hext.flow.tests.concurrent.TestDispatcher
{
    /**
     * @{inherit}
     */
    ...
    {
        this.dispatcher = new Dispatcher<Int>(new hext.threading.ThreadExecutor());
    }
    ...
    {
        this.dispatcher.register("event");
        assertEquals(this.dispatcher.trigger("event", 0).status, hext.flow.Dispatcher.Status.TRIGGERED);
    }
