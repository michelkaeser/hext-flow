...
/**
 * TestSuite for the hext.flow.async.Promise class.
 *
 * TODO: async specific tests
 * TODO: mock ThreadExecutor
 */
class TestPromise extends hext.flow.tests.concurrent.TestPromise
{
    /**
     * @{inherit}
     */
    ...
    {
        this.promise = new Promise<Int>(new hext.threading.ThreadExecutor());
    }

    /**
     * @{inherit}
     */
    override private function getPromise(resolves:Int = 1):hext.flow.async.Promise<Dynamic>
    {
        return new Promise<Dynamic>(new hext.threading.ThreadExecutor(), resolves);
    }
